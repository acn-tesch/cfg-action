...
grep -r --exclude="entrypoing.sh" 'AWSTemplateFormatVersion' ./* |cut -d':' -f1 > /tmp/cfn_templates
count=0
mkdir /tmp/cfn_guard_temp
for x in $(cat /tmp/cfn_templates); do
    echo "Scanning file - ${x}"
    cfn-guard -vt ${x} -r /rules/cfn-guard-sg-rule-testing.ruleset > /tmp/cfn_guard_temp/results${count}.json
